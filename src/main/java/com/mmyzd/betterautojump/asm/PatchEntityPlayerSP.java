...
public class PatchEntityPlayerSP extends BasePatch {
		...
		if (stateUpdateAutoJump == 0) {
			if (node instanceof MethodInsnNode) {
				...
				if (methodInsnNode.getOpcode() == Opcodes.INVOKEVIRTUAL
					...
					stateUpdateAutoJump++;
					return new AbstractInsnNode[] { new MethodInsnNode(
							Opcodes.INVOKESTATIC, ASMHelper.getIntlFromType("com.mmyzd.betterautojump.asm.MethodHook"),
							"disallowAutoJump",
	...
	private int stateOnLivingUpdate = 88;
	...
	public AbstractInsnNode[] patchOnLivingUpdate(AbstractInsnNode node) {
		if (stateOnLivingUpdate == 0 || stateOnLivingUpdate == 3) {
			...
			stateOnLivingUpdate++;
			return new AbstractInsnNode[] { node, new InsnNode(Opcodes.POP),
					new MethodInsnNode(Opcodes.INVOKESTATIC,
							ASMHelper.getIntlFromType("com.mmyzd.betterautojump.asm.MethodHook"),
							"isMovingModeEqualsToSprinting", ASMHelper.getDescFromType("boolean()"), false) };
